...
num_validation = 1080					# will be 2880 in full set
num_test = 3240
			...
			for width in widths:
				for noise in noises:
					for s in range(2):
						...
						original_file = path + 'original_images5/oim_' + "{:.2g}".format(sigma*1e-09) + '_' + str(alpha) + '_' + "{0:.2g}".format(Xi*1e-09) + '_' + str(width) + '_' + str(space) + '_' + str(-shift) + '.tiff'
						noisy_file = path + 'noisy_images5/nim_' + "{0:.2g}".format(sigma*1e-09) + '_' + str(alpha) + '_' + "{0:.2g}".format(Xi*1e-09) + '_' + str(width) + '_' + str(space) + '_' + str(-shift) + '_' + str(noise) + '.tiff'
...
for epoch in range(1,epochs+1):
	...
	for alpha in alphas:
		...
		print("Training epoch :",epoch,'number of training_images:',count)
	...
	if val_score < prev_val_score:
		model.save(path + 'models/' + 'Linenet_image5_run2_round_L2_rotation_epoch_'+ str(epoch) + '.h5')
		prev_val_score = val_score
	else:
		print('Validation score did not improve on epoch:',epoch)
