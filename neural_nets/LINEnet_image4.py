...
num_validation = 2520					# will be 2880 in full set
num_test = 7560
			...
			for width in widths:
				for noise in noises:
					for s in range(2):
						...
						noisy_file = path + 'noisy_images4/nim_' + "{0:.2g}".format(sigma*1e-09) + '_' + str(alpha) + '_' + "{0:.2g}".format(Xi*1e-09) + '_' + str(width) + '_' + str(space) + '_' + str(-shift) + '_' + str(noise) + '.tiff'
...
"""
model = load_model(path + 'models/' + 'Linenet_image4_round_L2_rotation_epoch_2.h5')
model.summary()
...
for epoch in range(3,epochs+1):
	...
	model.save(path + 'models/' + 'Linenet_image4_round_L2_rotation_epoch_'+ str(epoch) + '.h5')
