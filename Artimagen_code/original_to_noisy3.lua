         ...
         for Xi = 6, 40, 1 do
            for width = 20, 30, 10 do
		     ...
		     shift = math.floor(-25 + (width + space/2 + Xi + alpha*10 + sigma*10)%16)
                     local input_file = path .. 'original_images3/oim_' .. tostring(sigma*1e-09) .. '_' .. tostring(alpha) .. '_' .. tostring(Xi*1e-09) .. '_' .. tostring(width) .. '_' .. tostring(space) ..'_'..tostring(-shift)..'.tiff'
		     ...
		     --aig_apply_noise(im, "gaussian", 0.6)
                     ...
                     local output_file = path .. 'noisy_images3/nim_' .. tostring(sigma*1e-09) .. '_' .. tostring(alpha) .. '_' .. tostring(Xi*1e-09) .. '_' .. tostring(width) .. '_' .. tostring(space) ..'_'..tostring(-shift).. '_' ..tostring(noise)..'.tiff'
